...
6. ~~The model boundary does not specify whether dataset-dependent values like `input_channels` and `num_classes` are hardcoded in `model.cpp`, read from TOML, or passed in by the trainer, and it also does not define how experiment identity survives when `model.cpp` is copied into a production repo.~~ **Resolved by final decision:** The documented experiment-owned model entrypoint is `build_model(int64_t input_channels, int64_t num_classes)`. Those values come from resolved dataset metadata through the trainer boundary rather than authored TOML architecture fields or hardcoded constants. Each `model.cpp` should also carry a file-scope `constexpr std::string_view kExperimentId` provenance constant; retaining it after a production copy is recommended but not mandatory. Canonical IDs: `REQ-020`, `REQ-021`, `CON-013`, `CON-015`, `ADR-0002`, `ADR-0007`, `ADR-0008`, `ADR-0009`, `STAGE-02`, `STAGE-05`.
...
1. ~~FPGA constraint enforcement is tied to `fpga_int8_v1`, but the plan does not define how multiple future FPGA profiles should compose or inherit constraints.~~ **Resolved by final decision:** FPGA deploy profiles stay flat and explicit. They do not inherit or compose implicitly, and any new FPGA profile is introduced as a new full named rule set. Canonical IDs: `REQ-012`, `CON-010`, `UNK-003`, `ADR-0012`, `STAGE-09`.
...
6. The documented experiment-owned model entrypoint is `build_model(int64_t input_channels, int64_t num_classes)`. Dataset-dependent values come from resolved dataset metadata through the trainer boundary rather than authored TOML architecture fields or hardcoded constants, and each `model.cpp` should carry a file-scope `constexpr std::string_view kExperimentId` provenance constant for cross-repo traceability.
...
1. FPGA deploy profiles stay flat and explicit. There is no inheritance or profile composition.
